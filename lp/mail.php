...
$from = "bestroadofmajor@gmail.com";
...
.form-title{
	background-image: url(./library/images/contact-page.png);
...
<?php }else{ ?>
	<div class="form-title">
                <h1  class="form-title-h1">Contact</h1>
                <h2  class="form-title-h2">お問い合わせ</h2>
            </div>
<p class="confirmation">入力内容の確認</p>
...
<p class="policy">個人情報の取り扱いに関して</p>
<p class="policy-check">個人情報の取り扱いに関する内容に同意する</p>
<div class="btn-box">
	<input  class="mail-btn" type="button" value="修正する" onClick="history.back()">
	<input  class="mail-btn" type="submit" value="　送信する　"></p>
</div>
</form>
<?php } ?>
</div><!-- /formWrap -->
...
function confirmOutput($arr){
    ...
    $html = '';
    $last_key = key(array_slice($arr, -1, 1, true)); // 配列の最後のキーを取得
    ...
    foreach($arr as $key => $val) {
        ...
        // 最後の「お問い合わせ内容」の行に特別なクラスを適用する条件分岐
        if ($key === 'お問い合わせ内容' && $key === $last_key) {
            $html .= "<tr><th class='sample last-element'>".$key."</th><td class='sample-text last-element'>".$out."</td></tr>\n";
        } else {
            $html .= "<tr><th class='sample'>".$key."</th><td class='sample-text'>".$out."</td></tr>\n";
        }
